...
public class RunnerSave13 {
    public static void main(String[] args) {
        ...
        Configuration con = new org.hibernate.cfg.Configuration().configure("hibernate.cfg.xml").addAnnotatedClass(Student13.class);
        SessionFactory sf = con.buildSessionFactory();
        Session session = sf.openSession();
        Transaction tx = session.beginTransaction();
        session.save(student1);//Persistent state

        student1.setName("Ayse Arzu Hanim");//DB de de degisir!!!

        session.evict(student1);//!!!detached state yukaridaki degisikligin kaydolmasini engeller
